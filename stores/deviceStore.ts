...
export const useDeviceStore = defineStore("device", {
    ...
    filteredDevices(): DeviceHardware[] {
      if (this.tag) {
        return this.targets.filter(
          (t) =>
            t.tags?.includes(this.tag ?? "") || t.architecture === this.tag,
        );
      }
      return this.targets;
    },
  ...
  actions: {
    ...
    async setSelectedTarget(target: DeviceHardware) {
      ...
      if (
        !firmwareStore.hasFirmwareFile &&
        !firmwareStore.hasOnlineFirmware &&
        firmwareStore.stable.length > 0
      ) {
    ...
    async enterDfuMode(tFunc?: (key: string) => string) {
      ...
      const isPortBusy =
        firmwareStore.isConnected ||
        firmwareStore.isReaderLocked ||
        serialMonitorStore.isConnected ||
        serialMonitorStore.isReaderLocked;
      if (isPortBusy) {
        const errorTitle = tFunc?.("serial.busy_title") || "Serial Port Busy";
        const errorMessage =
          tFunc?.("serial.busy_message") ||
          "A serial connection is already open. Please close it before starting DFU mode.";
      ...
      try {
        ...
        try {
          ...
          if ((error as Error).message === "timeout") {
            const errorTitle =
              tFunc?.("dfu.error_connection_title") ||
              "Device Connection Failed";
            const errorMessage =
              tFunc?.("dfu.error_connection") ||
              "Failed to connect to device. Please disconnect and reconnect the device, then try again. If the problem persists, reload the page.";
            toastStore.error(errorTitle, errorMessage);
            throw error;
          } else {
            throw error;
        ...
        const successMessage =
          tFunc?.("dfu.success_message") ||
          "Device successfully entered DFU mode";
        toastStore.success(successTitle, successMessage);
      } catch (error: any) {
        console.error("Error entering DFU mode:", error);

        const errorTitle =
          tFunc?.("dfu.error_connection_title") || "Device Connection Failed";
        const errorMessage =
          tFunc?.("dfu.error_connection") ||
          "Failed to connect to device. Please disconnect and reconnect the device, then try again. If the problem persists, reload the page.";
    ...
    async autoSelectHardware(tFunc?: (key: string) => string) {
      ...
      if (isPortBusy) {
        ...
        const errorMessage =
          tFunc?.("serial.busy_message") ||
          "A serial connection is already open. Please close it before auto-detecting hardware.";
        toastStore.error(errorTitle, errorMessage);
        return;
      }
      this.isConnecting = true;
      try {
        ...
        device.events.onDeviceMetadataPacket.subscribe(async (packet: any) => {
          ...
          if (packet?.data?.platformioTarget?.length > 0) {
            targetDevice = this.targets.find(
              (target: DeviceHardware) =>
                target.platformioTarget === packet?.data?.platformioTarget,
            );
          }
          if (!targetDevice) {
            targetDevice = this.targets.find(
              (target: DeviceHardware) =>
                target.hwModel === packet?.data?.hwModel,
            );
          }
          if (targetDevice) {
        ...
        try {
          ...
          if ((error as Error).message === "timeout") {
            ...
            const errorTitle =
              tFunc?.("dfu.error_unresponsive_title") || "Device Unresponsive";
            const errorMessage =
              tFunc?.("dfu.error_unresponsive") ||
              "The device is not responding. Please ensure it is properly connected and not in DFU mode.";
            toastStore.error(errorTitle, errorMessage);
            throw error;
          } else {
            throw error;
